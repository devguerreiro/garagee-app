...
import { ParkingSpaceBookingsDTO, BookingStatusDTO } from "@/app/dtos";
...
export default function Feed({ status, bookings }: Readonly<Props>) {
  ...
  function handleTabChange(value: string) {
    if (value === "approved") replace("/garagem/solicitacoes?status=approved");
    else if (value === "pending")
      replace("/garagem/solicitacoes?status=pending");
    else if (value === "refused")
      replace("/garagem/solicitacoes?status=refused");
    else if (value === "revoked")
      replace("/garagem/solicitacoes?status=revoked");
    else replace("/garagem/solicitacoes");
  }
